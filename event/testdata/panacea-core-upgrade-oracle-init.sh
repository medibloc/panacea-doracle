...
panacead add-genesis-oracle --oracle-unique-id ${UNIQUE_ID} --oracle-account validator
...
# Open a 1317 REST port publicly
...
sed -i 's|^"voting_period" = .*|voting_period = "15s"|g' ~/.panacea/config/genesis.json
sed -i 's|^"voting_period" = .*|voting_period = "15s"|g' ~/.panacea/config/genesis.json
